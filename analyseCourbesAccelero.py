...
indexes = detect_peaks(acc, mph=750 , mpd=1000)
...
for i in range (len(indexes)):
    plt.ylabel('accele')
    plt.plot(t[indexes[i]-200:indexes[i]+200],acc[indexes[i]-200:indexes[i]+200])
    plt.show()
    from numpy.fft import fft
    T= t[indexes[i]+200] - t[indexes[i]-200] 
    fe = 1000
    nfft = np.arange(start=0.0,stop=4,step=1.0/fe)
    
    echantillons = acc[indexes[i]-200:indexes[i]+200]
