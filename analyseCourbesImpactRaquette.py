...
import scipy.io
mat = scipy.io.loadmat('impact.mat')

## ========================================================================
# cherche le nombre de tests et les intervales d'interet
# =========================================================================
#mesureRaquette=mesures{1};  # capteur cote raquette et mesures telles que vitesse positive a t=0
#t=mesureRaquette(:,1)*1e-6; # en sec
#ang=deg2rad(mesureRaquette(:,2)); # en rad
mesureRaquette=mat["mesureRaquette"] # provient de 'Acquisition'
print ('lenmesuresraquette =', len(mesureRaquette))
#t = mesureRaquette[][0]*1e-6 # en secondes
...
for line in mesureRaquette:
    ...
    try :
        ...
        # fonction de transformation bit to degre
        if rating == 20: 
#            transAng=(val*360/((2^16)*0.8)); # le 0.8 est du a la bande de 10# vers 90# du capteur
            a = np.append(a, -1*np.radians(val))
            t = np.append(t, tmp*1e-6)
#            print (val , np.radians(val))
            rating  = 0
print ("finish" )
import time
